...
export async function fetchActuacionesFile ( idProceso: number ) {
  ...
  if ( fsnp.existsSync( `actuaciones/${ idProceso }` ) ) {
    ...
    fsp.appendFile( `actuaciones/${ idProceso }/actuaciones.json`,
      actuacionesRaw );
  }
  else {
    ...
    setTimeout( () => {
      ...
      radicado.procesos.forEach( ( proceso, i ) => {
        ...
        setTimeout( () => {
          ...
          fetchActuacionesFile( idProceso );
        },
          i * 10000 );
      } );
    },
...
export async function getActuaciones ( idProceso: number, llaveProceso: string ) {
  const res = await fetch(
    `${ getBaseUrl() }/Procesos/${ llaveProceso }/Actuaciones?idProceso=${ idProceso }`
  );
  if ( !res.ok ) {
  ...
  const actuaciones = ( await res.json() ) as intActuacion[];
